...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::Search(const KeyType &key, const KeyComparator &comparator) const -> int {
  auto cmp = [&](const MappingType &p, const KeyType &v) { return comparator(p.first, v) < 0; };
  auto index = std::lower_bound(array_, array_ + GetSize(), key, cmp) - array_;
  return index == GetSize() || comparator(array_[index].first, key) != 0 ? -index - 1 : index;
}
